  ...
  def link_like_params(object,options)
    ...
    if !user_signed_in?
      ...
      if (object.liked_by?(current_subject))
        ...
        params << [object, :like]
        params << {:class => "verb_like like_size_" + options[:size].to_s,:id => "like_" + dom_id(object),:method => :delete, :remote => true}
      else
        params << if options[:size] == :small
                    image_tag("star-off10.png", :class => "menu_icon")
                  else
        ...
        params << [object, :like]
        params << {:class => "verb_like like_size_" + options[:size].to_s,:id => "like_" + dom_id(object),:method => :post, :remote => true}
      end
    end
  end
