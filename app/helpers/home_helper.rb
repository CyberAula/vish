module HomeHelper
  ...
  def subject_content(subject, klass, options = {})
    ...
    query = query.joins(:excursions).where("excursions.draft == false") if (klass.include?(Excursion)) and (options[:scope] == :net or options[:scope] == :more or (subject != current_subject and options[:scope] == :me))
