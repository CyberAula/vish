SocialStream.setup do |config|
  ...
  #
  config.objects = [ :post, :comment, :document, :link, :excursion, :slide, :embed, :swf, :officedoc, :event, :category, :scorm ]
  ...
  # 
  config.quick_search_models = [:excursion, :user, :picture, :video, :audio, :swf, :officedoc, :document, :embed, :link, :event, :scorm]
  config.extended_search_models = [:excursion, :user, :event, { :resource => [ :picture, :video, :audio, :swf, :officedoc, :document, :embed, :link, :scorm ] } ]
