...
namespace :fix do
  ...
  #Usage
  #Development:   bundle exec rake fix:harvestILSs
  #In production: bundle exec rake fix:harvestILSs RAILS_ENV=production
  task :harvestILSs => :environment do
    ...
    parsed_content.each do |ils|
      ...
      unless ils["ils_license"].blank? or l.original_author.blank? or l.url.blank?
        license = processILSLicense(ils["ils_license"])
        l.license_id = License.find_by_key(license).id unless license.nil? rescue nil
